...
impl McpLoopConfig {
    ...
    pub fn from_request(max_tool_calls: Option<i32>) -> Self {
        Self {
            max_iterations: 10,
...
pub trait McpLoopBackend: Send + Sync {
    ...
    fn build_resume_request(&self, base_request: &Self::Request) -> Result<Self::Request, Self::Error>;
    ...
    /// Build response that includes both executed MCP results and pending function calls
    ///
    /// Called when function tools are detected - MCP tools are executed first,
    /// then the response is returned with function tools for the caller to handle
    fn build_mixed_tools_response(
        &self,
        response: Self::Response,
        mcp_results: &[ToolExecutionResult],
        function_calls: &[ParsedToolCall],
    ) -> Self::Response;
}
...
pub async fn execute_mcp_loop<B: McpLoopBackend>(
    ...
    loop {
        ...
        if iteration > config.max_iterations {
            ...
            return Err(backend.execute_iteration(&current_request).await.err().unwrap());
        }
        ...
        match result {
            IterationOutcome::Completed(mut response) => {
                ...
                if total_calls > 0 {
                    backend.inject_mcp_metadata(&mut response, mcp_manager, total_calls);
                }
            ...
            IterationOutcome::ToolCallsFound(tool_calls) => {
                ...
                // Execute MCP tools
                let mut mcp_results = Vec::with_capacity(mcp_calls.len());
                ...
                if !function_calls.is_empty() {
                    ...
                    let result = backend.execute_iteration(&current_request).await?;
                    if let IterationOutcome::Completed(response) = result {
                        let mut final_response = backend.build_mixed_tools_response(
                            response,
                            &mcp_results,
                            &function_calls,
                        );
                        if total_calls > 0 {
                            backend.inject_mcp_metadata(&mut final_response, mcp_manager, total_calls);
                        }
                ...
                backend.prepare_request_with_mcp_tools(&mut current_request, &mcp_tools, iteration + 1);
            }
        }
    }
}
...
async fn execute_single_tool(
    ...
    let result = mcp_manager.call_tool(&call.name, call.arguments.as_str()).await;
    ...
    let (output, success, error) = match result {
        Ok(value) => match serde_json::to_string(&value) {
            Ok(output) => {
                Metrics::record_mcp_tool_call(model_name, &call.name, metrics_labels::RESULT_SUCCESS);
                (output, true, None)
            }
            Err(e) => {
...
/// Partition tool calls into MCP and function tools
///
/// This is a convenience function for backends that need to partition
/// tool calls before processing.
...
mod tests {
    ...
    #[test]
    fn test_config_effective_limit() {
        // Default config
        let config = McpLoopConfig::default();
        assert_eq!(config.effective_limit(), 10);

        // User limit lower than safety
        let config = McpLoopConfig {
            max_iterations: 10,
            max_tool_calls: Some(5),
        };
        assert_eq!(config.effective_limit(), 5);

        // User limit higher than safety
        let config = McpLoopConfig {
            max_iterations: 10,
    ...
    fn test_partition_tool_calls() {
        ...
        let mcp_names: HashSet<String> = ["mcp_tool".to_string()].into_iter().collect();
        let (mcp, func) = partition_tool_calls(calls, &mcp_names);

        assert_eq!(mcp.len(), 2);
        assert_eq!(func.len(), 1);
        assert_eq!(func[0].name, "function_tool");
    }
}
